...
class BlueHorse extends Horse {

    //set up Start, End, Home, HorseNumber and NestPosition
    public BlueHorse(int horseNum) {
        horseNumber = horseNum;
        color = "blue";
        setStartPos(0);
        setHomeArrivalPos(47);
        setHomePos(new int[]{64, 65, 66, 67, 68, 69});
        setHorseNumber(horseNumber);
        resetHorse();
    }

    //bring horse back to nest when new game
    public void resetHorse() {
        setNestPos(horseNumber, new int[]{48, 49, 50, 51});
    }

    //check regular move for blue horse
    public boolean checkRegularMove(int diceResult) {
        ...
        if (currentPos + diceResult > 47) {return false;}
        else {
            nextPos = currentPos + diceResult;
            return true;
        }
    }
